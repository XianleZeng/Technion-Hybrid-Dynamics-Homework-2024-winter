...
fcn_name='v_wheel';
fid=fopen([fcn_name,'.m'],'w');
fprintf(fid,'function [v_wheel_1, v_wheel_2, v_wheel_3]=%s',fcn_name);
fprintf(fid,'(q, q_d)\n');
fprintf(fid,'%% Xianle Zeng\n');
fprintf(fid,'%% %s\n\n',datestr(now));
...
%
fprintf(fid,'\n%% v_wheel_1\n');
fprintf(fid, 'v_wheel_1=zeros(2,1);\n');
for k=1:2
	if v_wheel_1(k)~=0
		ttt=char(v_wheel_1(k));
		fprintf(fid,'v_wheel_1(%s)=%s;\n',num2str(k),ttt);
	end
end

fprintf(fid,'\n%% v_wheel_2\n');
fprintf(fid, 'v_wheel_2=zeros(2,1);\n');
for k=1:2
	if v_wheel_2(k)~=0
		ttt=char(v_wheel_2(k));
		fprintf(fid,'v_wheel_2(%s)=%s;\n',num2str(k),ttt);
	end
end
